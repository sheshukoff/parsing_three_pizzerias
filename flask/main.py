    ...
    if request.method == 'GET':
        ...
        if not input_login or not input_password:
            ...
            if input_login == login_user_db and is_valid is True:
                ...
                return redirect(url_for('choose_brand', login=input_login))
            else:
                flash('Логин или пароль не правильный')
    else:
        print('another_method')
...
def sign_up():
    if request.method == 'GET':
        ...
        if not (login or password or password_repeat):
            ...
            # сдесь происходит загрузка в базу данных login и password (пароль уже хеширован)
...
app.add_url_rule("/choose_brand", 'choose_brand', choose_brand, methods=['GET', 'POST'])
app.add_url_rule("/choose_brand/choose_city", 'choose_city', choose_city, methods=['GET', 'POST'])
